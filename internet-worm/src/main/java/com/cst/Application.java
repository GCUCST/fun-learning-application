...
public class Application {
    ...
    public static void showTarget(String baseCurrency, String side, double triggerMoney) throws Exception {
        String url = "https://www.okexcn.com/v3/c2c/otc-ticker/quotedPrice?t=%s&baseCurrency=%s&quoteCurrency=CNY&side=%s";
        ...
        if (side.equals(SELL) && Double.parseDouble(jsonArray.getJSONArray("data").getJSONObject(0).getString("price")) > triggerMoney) {
            System.err.println("该卖了大于啦");
            Mail.buildAndSendMsg("该卖了大于啦：" + result);
        }
        if (side.equals(BUY) && Double.parseDouble(jsonArray.getJSONArray("data").getJSONObject(0).getString("price")) < triggerMoney) {
